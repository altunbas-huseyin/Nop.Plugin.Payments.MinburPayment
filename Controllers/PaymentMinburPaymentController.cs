...
using Nop.Plugin.Payments.MinburPayment.Models;
...
namespace Nop.Plugin.Payments.MinburPayment.Controllers
{
    [AutoValidateAntiforgeryToken]
    public class PaymentMinburPaymentController : BasePaymentController
    {
        #region Fields
        ...
        public PaymentMinburPaymentController(IGenericAttributeService genericAttributeService,
        ...
        public IActionResult Configure()
        {
            ...
            var MinburPaymentPaymentSettings = _settingService.LoadSetting<MinburPaymentPaymentSettings>(storeScope);

            var model = new ConfigurationModel
            {
                UseSandbox = MinburPaymentPaymentSettings.UseSandbox,
                BusinessEmail = MinburPaymentPaymentSettings.BusinessEmail,
                PdtToken = MinburPaymentPaymentSettings.PdtToken,
                PassProductNamesAndTotals = MinburPaymentPaymentSettings.PassProductNamesAndTotals,
                AdditionalFee = MinburPaymentPaymentSettings.AdditionalFee,
                AdditionalFeePercentage = MinburPaymentPaymentSettings.AdditionalFeePercentage,
                ActiveStoreScopeConfiguration = storeScope
            };

            if (storeScope <= 0)
                return View("~/Plugins/Payments.MinburPayment/Views/Configure.cshtml", model);

            model.UseSandbox_OverrideForStore = _settingService.SettingExists(MinburPaymentPaymentSettings, x => x.UseSandbox, storeScope);
            model.BusinessEmail_OverrideForStore = _settingService.SettingExists(MinburPaymentPaymentSettings, x => x.BusinessEmail, storeScope);
            model.PdtToken_OverrideForStore = _settingService.SettingExists(MinburPaymentPaymentSettings, x => x.PdtToken, storeScope);
            model.PassProductNamesAndTotals_OverrideForStore = _settingService.SettingExists(MinburPaymentPaymentSettings, x => x.PassProductNamesAndTotals, storeScope);
            model.AdditionalFee_OverrideForStore = _settingService.SettingExists(MinburPaymentPaymentSettings, x => x.AdditionalFee, storeScope);
            model.AdditionalFeePercentage_OverrideForStore = _settingService.SettingExists(MinburPaymentPaymentSettings, x => x.AdditionalFeePercentage, storeScope);
        ...
        public IActionResult Configure(ConfigurationModel model)
        {
            ...
            //save settings
            MinburPaymentPaymentSettings.UseSandbox = model.UseSandbox;
            MinburPaymentPaymentSettings.BusinessEmail = model.BusinessEmail;
            MinburPaymentPaymentSettings.PdtToken = model.PdtToken;
            MinburPaymentPaymentSettings.PassProductNamesAndTotals = model.PassProductNamesAndTotals;
            MinburPaymentPaymentSettings.AdditionalFee = model.AdditionalFee;
            MinburPaymentPaymentSettings.AdditionalFeePercentage = model.AdditionalFeePercentage;

            /* We do not clear cache after each setting update.
             * This behavior can increase performance because cached settings will not be cleared 
             * and loaded from database after each update */
            _settingService.SaveSettingOverridablePerStore(MinburPaymentPaymentSettings, x => x.UseSandbox, model.UseSandbox_OverrideForStore, storeScope, false);
            _settingService.SaveSettingOverridablePerStore(MinburPaymentPaymentSettings, x => x.BusinessEmail, model.BusinessEmail_OverrideForStore, storeScope, false);
            _settingService.SaveSettingOverridablePerStore(MinburPaymentPaymentSettings, x => x.PdtToken, model.PdtToken_OverrideForStore, storeScope, false);
            _settingService.SaveSettingOverridablePerStore(MinburPaymentPaymentSettings, x => x.PassProductNamesAndTotals, model.PassProductNamesAndTotals_OverrideForStore, storeScope, false);
            _settingService.SaveSettingOverridablePerStore(MinburPaymentPaymentSettings, x => x.AdditionalFee, model.AdditionalFee_OverrideForStore, storeScope, false);
            _settingService.SaveSettingOverridablePerStore(MinburPaymentPaymentSettings, x => x.AdditionalFeePercentage, model.AdditionalFeePercentage_OverrideForStore, storeScope, false);
        ...
        public IActionResult RoundingWarning(bool passProductNamesAndTotals)
        {
            ...
            if (passProductNamesAndTotals && !_shoppingCartSettings.RoundPricesDuringCalculation)
                return Json(new { Result = _localizationService.GetResource("Plugins.Payments.MinburPayment.RoundingWarning") });
        ...
        public IActionResult PDTHandler()
        {
            ...
            if (!(_paymentPluginManager.LoadPluginBySystemName("Payments.MinburPayment") is MinburPaymentPaymentProcessor processor) || !_paymentPluginManager.IsPluginActive(processor))
            ...
            if (processor.GetPdtDetails(tx, out var values, out var response))
            {
                ...
                var newPaymentStatus = MinburHelper.GetPaymentStatus(paymentStatus, string.Empty);
                ...
                //validate order total
                var orderTotalSentToPayPal = _genericAttributeService.GetAttribute<decimal?>(order, MinburHelper.OrderTotalSentToPayPal);
                ...
                if (orderTotalSentToPayPal.HasValue)
                    _genericAttributeService.SaveAttribute<decimal?>(order, MinburHelper.OrderTotalSentToPayPal, null);
        ...
        public IActionResult IPNHandler()
        {
            ...
            var newPaymentStatus = MinburHelper.GetPaymentStatus(paymentStatus, pendingReason);
            sb.AppendLine("New payment status: " + newPaymentStatus);
